...
def get_issueblog():
    global issueblog
    issueblog = user.get_repo('%s/issueblog' % user.get_user().login)
...
def bundle_summary_section():
    ...
    total_label_count = issueblog.get_labels().totalCount
    total_issue_count = issueblog.get_issues().totalCount
    labels_html_url = 'https://github.com/%s/issueblog/labels' % user.get_user().login
    issues_html_url = 'https://github.com/%s/issueblog/issues' % user.get_user().login
...
def bundle_pinned_issues_section():
    ...
    pinned_label = issueblog.get_label(':+1:置顶')
    pinned_issues = issueblog.get_issues(labels=(pinned_label,))
...
def format_issue_with_labels(issue: Issue):
    ...
    for label in labels:
        labels_str += sub('[%s](https://github.com/%s/issueblog/labels/%s)\t|\t' % (
...
def bundle_new_created_section():
    ...
    new_5_created_issues = issueblog.get_issues()[:5]
...
def bundle_list_by_labels_section():
    ...
    all_labels = issueblog.get_labels()
    ...
    for label in all_labels:
        ...
        # 所有的issue数量, 之后再优化.
        count = 0
        issues_in_label = issueblog.get_issues(labels=(label,))
